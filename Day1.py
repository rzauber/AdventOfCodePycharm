...
map = {'nine':'9', 'eight':'8', 'seven':'7', 'six':'6', 'five': '5', 'four':'4','three':'3','two':'2','one':'1','zero':'0'}
...
for l in input.split('\n'):
  ...
  for val in map:
    a = l.find(val)
    b = l.find(map[val])
    pos = 99999 if (a == -1 and b == -1) else b if (a == -1) else a if (b == -1) else (min(a,b))
    if pos < minPos:
      minPos = pos
      minVal = int(map[val])
    pos = max(l.rfind(val), l.rfind(map[val])) 
    if pos > maxPos:
      maxPos = pos
      maxVal = int(map[val])
...
print(sum)  
  #.replace('nine','9').replace('eight','8').replace('seven','7').replace('six','6').replace('five','5').replace('four','4').replace('three','3').replace('two','2').replace('one','1').replace('zero','0')
  #d = []
  #for c in l:
  #  if c.isdigit(): 
  #    d.append(c)
  #if d:
  #  sum += int(d[0]) * 10 + int(d[-1])
  #else:
  #  print(l)
  #print(l + str(d[0]) + ' ' + str(d[-1]))
#print(str(sum))